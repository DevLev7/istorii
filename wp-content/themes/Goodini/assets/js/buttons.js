...
(function() {
	...
	*/
	...
	tinymce.PluginManager.add('text_middle', function( editor, url ) {
		editor.addButton( 'text_middle', { 
			text: '1.25',
			//icon: 'text_middle',
			title: 'Средний текст',
			onclick: function() {
				editor.selection.setContent('<span class="text_middle">' + editor.selection.getContent() + '</span>');
			}
		});
	});
	
	tinymce.PluginManager.add('text_big', function( editor, url ) {
		editor.addButton( 'text_big', { 
			text: '1.5',
			//icon: 'text_big',
			title: 'Крупный текст',
			onclick: function() {
				editor.selection.setContent('<span class="text_big">' + editor.selection.getContent() + '</span>');
			}
		});
	});
	
	tinymce.PluginManager.add('text_bigger', function( editor, url ) {
		editor.addButton( 'text_bigger', { 
			text: '2',
			//icon: 'text_big',
			title: 'Большой текст',
			onclick: function() {
				editor.selection.setContent('<span class="text_bigger">' + editor.selection.getContent() + '</span>');
			}
		});
	});
		...
		editor.addButton( 'popup_buttons', { 
			...
			menu: [ 
				...
				},
				{
					text: 'Кнопка-ссылка',
					onclick: function() {
					editor.insertContent('[btn link=""]Скачать прайс[/btn]');
					}
				},
				{
					text: 'Квиз (marquiz)',
					onclick: function() {
						editor.insertContent('[marquiz marquiz="id-квиза"]Пройти тест[/marquiz]');
					}
				},
			]
		});
