...
//        $term_names = array_map( function ($path) {
...
//        }, $term_names );
     ...
     * @return bool|string
     */
    public function postTypeOfCurrentTax()
    {
        if ( !is_tax() ) {
            return false;
        }
        ...
        if ( is_a( $term, 'WP_Term' ) ) {
            foreach ($this->taxonomies as $post_type_name => $taxonomies) {
    ...
    public function permalink($permalink, $post)
    {
        ...
        foreach ($this->taxonomies[$post->post_type] as $tax_name) {
            ...
            $permalink = str_replace( "%$tax_name%", trim( $taxonomy_slug, '/' ), $permalink );
        }
        return $permalink;
    }
