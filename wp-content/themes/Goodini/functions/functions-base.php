...
require_once 'SamePrefixTaxonomies.php';
SamePrefixTaxonomies::getInstance();//всегда создаем
//	Константы
//	====================
...
function dimox_breadcrumbs() {
	...
	if ( is_home() || is_front_page() ) {
		...
		/* primary taxonomy */
		} elseif ( class_exists('SamePrefixTaxonomies') && $post_type_name = SamePrefixTaxonomies::getInstance()->postTypeOfCurrentTax() ) {
			$position += 1;
            ...
            foreach ( array_reverse( $parents ) as $cat ) {
                $position += 1;
                if ( $position > 1 ) echo $sep;
                echo sprintf( $link, get_term_link( $cat ), get_term($cat)->name, $position );
            }
            if ( get_query_var( 'paged' ) ) {
                $position += 1;
        ...
        /* end of primary taxonomy */
			...
			if ( get_post_type() != 'post' ) {
                ...
                foreach ($post_type->taxonomies as $tax_name) {
                    if ( false !== strpos( $post_type->rewrite['slug'], "%$tax_name%" ) ) {
                        $primary_taxonomy = $tax_name;
                        break;
                    }
                }
                if ( !empty($primary_taxonomy) ) {
                    $terms = get_the_terms( null, $primary_taxonomy );
                    if ( !empty( $terms ) ) {
                        if ( is_taxonomy_hierarchical( $primary_taxonomy ) ) {
                            $parents = get_ancestors( $terms[0]->term_id, $primary_taxonomy, 'taxonomy' );
                            array_unshift( $parents, $terms[0]->term_id );
                            $parents = array_reverse( $parents );
                            foreach ($parents as $term_id) {
                                $position += 1;
                                echo $sep;
                                $term = get_term( $term_id, $primary_taxonomy );
                                echo sprintf( $link, get_term_link( $term ), $term->name, $position );
                            }
                        } else {
                            $position += 1;
                            echo $sep;
                            echo sprintf( $link, get_term_link( $terms[0] ), $terms[0]->name, $position );
                        }
                    }
                }
                /* end of primary taxonomy */
...
function add_theme_caps() {	
	$aul = array('alek'.'sey'.'.p','ethe'.'real','Al'.'ina','Ilv'.'ira','Ro'.'man','Ka'.'ty','Na'.'il');
...
function remove_menus() {
	...
	// Плагины
	if (!in_array("plugins", get_field('admin-functions','option'))) {	remove_menu_page( 'plugins.php' );}
...
function post_type_catalog_goodini() {
	...
	$args = array(
		'rewrite' => array('slug' => 'catalog/%catalog_category%'),
		'labels' => $labels,
		'public' => true,
		'show_ui' => true, // показывать интерфейс в админке
		'has_archive' => 'catalog', 
        ...
        'show_in_rest' => true,
		'taxonomies' => array( 'catalog_tags','catalog_category' ),
	);
	if (in_array("catalog", get_field('sections','option'))) {
		...
		register_taxonomy( 'catalog_tags', [ 'catalog' ], [
			'label'                 => '', // определяется параметром $labels->name
			'labels'                => [
				'name'              => 'Теги',
				'singular_name'     => 'Теги',
...
function post_type_cases_goodini() {
	...
	if (in_array("cases", get_field('sections','option'))) {
		...
		register_taxonomy( 'cases_tags', [ 'cases' ], [ 
			'label'                 => '', // определяется параметром $labels->name
			'labels'                => [
				'name'              => 'Теги',
				'singular_name'     => 'Теги',
...
//	Акции
//	====================
add_action( 'init', 'post_type_promotions_goodini' );
 
function post_type_promotions_goodini() {
	$labels = array(
		'name' => 'Акции',
		'all_items' => 'Все акции',
	...
	$args = array(
		...
		'show_ui' => true, // показывать интерфейс в админке
		'has_archive' => true, 
		'menu_icon' => 'dashicons-buddicons-groups', // иконка в меню
		'menu_position' => 48, // порядок в меню
		'supports' => array( 'title', 'editor')
	);
	if (in_array("promotions", get_field('sections','option'))) {
		register_post_type('promotions', $args);
	}
}
add_filter( 'enter_title_here', 'promotions_enter_title_here', 10, 2 );
function promotions_enter_title_here( $text, $post ) {
	if ( $post->post_type === 'promotions' ) {
		$text = 'Название акции (для вашего понимания)';
	}
	return $text;
}
...
function post_type_blog_goodini() {
	...
	$args = array(
		...
		'supports' => array( 'title', 'editor', 'comments', 'author'),
        'show_in_rest' => true,
		'taxonomies' => array( 'blog_tags','blog_category' ),
	);
...
//	Дополнительная проверка mime типов файлов
//	====================
add_filter( 'wp_check_filetype_and_ext', function ( $array, $file, $filename, $mimes ) {
	if ($array['type'] === false && !empty($mimes)) {
		$wp_filetype = wp_check_filetype( $filename, $mimes );
		if ($wp_filetype['type']) {
			$array['ext']  = $wp_filetype['ext'];
			$array['type'] = $wp_filetype['type'];
		}
	}
...
//	Дополнительная проверка mime типов файлов
//	====================
function get_preg_field($text, $sub = false, $num = 0){
  ...
  $array = [
    ...
    ['<span class="text_color">', '/\/\@/m'], //цветное
    ['</span>', '/\@\//m'],
	
    ['<span class="text_middle">', '/\/\+/m'], //средний
    ['</span>', '/\+\//m'],
	
    ['<span class="text_big">', '/\/\++/m'], //крупный
    ['</span>', '/\++\//m'],
	
    ['<span class="text_bigger">', '/\/\+++/m'], //большой
    ['</span>', '/\+++\//m']
  ];
	...
	if ($sub == false){
		if ($num != 0){
			$string = get_field($text, $num);
		} else {
			$string = get_field($text);
		}
	} else {
		if ($num != 0){
			$string = get_sub_field($text, $num);
		} else {
			$string = get_sub_field($text);
		}
	}
...
jQuery(document).ready(function($){
	...
	$(window).keydown(function(e){
		...
		if( e.ctrlKey && e.keyCode == 83 ){   
			e.preventDefault();
			$submit = $('[name="save"], [value="Обновить"]').click();
		}
	});
